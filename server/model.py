...
class DecodeBox:
    ...
    def non_max_suppression(
        ...
        for i, image_pred in enumerate(prediction):
            ...
            conf_mask = (
                image_pred[:, 4] * class_conf[:, 0] >= conf_threshold
            ).squeeze()
...
class Yolo(object):
    ...
    def detect_heatmap(self, image_path):
        ...
        image_data = np.expand_dims(
            np.transpose(np.array(image_data, dtype="float32") / 255.0, (2, 0, 1)), 0
        )
        with torch.no_grad():
        ...
        for sub_output in outputs:
            ...
            sub_output = np.transpose(
                np.reshape(sub_output, [b, 3, -1, h, w]), [0, 3, 4, 1, 2]
            )[0]
        ...
        plt.savefig(
            f"data/cache/image/heatmap/{image_name}.png",
            dpi=200,
            bbox_inches="tight",
            pad_inches=-0.1,
        )
    ...
    def detect_image(self, image_path):
        ...
        with torch.no_grad():
            ...
            if results[0] is None:
                image.save(
                    f"data/cache/image/out/{image_name}.png", quality=95, subsampling=0
                )
        ...
        for i, c in list(enumerate(top_label)):
            ...
            for d in range(thickness):
                draw.rectangle(
                    (left + d, top + d, right - d, bottom - d), outline=self.colors[c]
                )
            draw.rectangle(
            ...
            draw.text(
                tuple(text_origin), str(label, "UTF-8"), fill=(0, 0, 0), font=font
            )
            del draw
...
class YoloDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if (
            ...
            image, box = self.get_random_data(
                self.annotation_lines[index], random=self.train
            )
    ...
    def get_random_data(
        self, annotation_line, jitter=0.3, hue=0.1, sat=0.7, val=0.4, random=True
    ):
    ...
    def get_random_data_with_mosaic(
        self, annotation_line, jitter=0.3, hue=0.1, sat=0.7, val=0.4
    ):
        h, w = 416, 416
...
class YoloLoss(nn.Module):
    ...
    def box_ciou(b1, b2):
        ...
        intersect_wh = torch.max(
            intersect_max - intersect_min, torch.zeros_like(intersect_max)
        )
        ...
        enclose_wh = torch.max(
            enclose_max - enclose_min, torch.zeros_like(intersect_max)
        )
    ...
    def forward(self, idx, _input, targets=None):
        ...
        prediction = (
            _input.view(bs, len(self.anchors_mask[idx]), self.bbox_attrs, in_h, in_w)
            .permute(0, 1, 3, 4, 2)
            .contiguous()
        )
        x = torch.sigmoid(prediction[..., 0])
        ...
        conf = torch.sigmoid(prediction[..., 4])
        pred_cls = torch.sigmoid(prediction[..., 5:])
        y_true, no_obj_mask, _ = self.get_target(
            idx, targets, scaled_anchors, in_h, in_w
        )
        no_obj_mask, pred_boxes = self.get_ignore(
            idx, x, y, h, w, targets, scaled_anchors, in_h, in_w, no_obj_mask
        )
        if torch.cuda.is_available():
        ...
        if n != 0:
            ...
            loss_cls = torch.mean(
                self.bce_loss(pred_cls[obj_mask], y_true[..., 5:][obj_mask])
            )
            loss += loss_loc * self.box_ratio + loss_cls * self.cls_ratio
        if self.focal_loss:
            pos_neg_ratio = torch.where(
                obj_mask,
                torch.ones_like(conf) * self.alpha,
            ...
            loss_conf = (
                torch.mean(
                    (
                        self.bce_loss(conf, obj_mask.type_as(conf))
                        * pos_neg_ratio
                        * hard_easy_ratio
                    )[no_obj_mask.bool() | obj_mask]
                )
                * self.focal_loss_ratio
            )
        else:
    ...
    def get_ignore(
        ...
        grid_x = (
            ...
            .repeat(in_h, 1)
            .repeat(int(bs * len(self.anchors_mask[idx])), 1, 1)
            .view(x.shape)
            .type_as(x)
        )
        grid_y = (
    ...
    def get_target(self, idx, targets, anchors, in_h, in_w):
        ...
        no_obj_mask = torch.ones(
            bs, len(self.anchors_mask[idx]), in_h, in_w, requires_grad=False
        )
        ...
        for b in range(bs):
            ...
            anchor_shapes = torch.FloatTensor(
                torch.cat(
                    (torch.zeros((len(anchors), 2)), torch.FloatTensor(anchors)), 1
                )
            )
            ...
            for t, best_n in enumerate(best_ns):
                ...
                box_loss_scale[b, k, j, i] = (
                    batch_target[t, 2] * batch_target[t, 3] / in_w / in_h
                )
