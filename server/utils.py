...
def draw_plot(
    dictionary, n_classes, window_title, plot_title, x_label, output_path, plot_color
):
...
def fit1epoch(
    ...
    val_loss = 0
    bar = tqdm(
        total=epoch_step, desc=f"epoch {e + 1}/{epoch}", postfix=dict, mininterval=0.3
    )
    model_train.train()
    ...
    if (e + 1) % period == 0 or e + 1 == epoch:
        torch.save(
            model.state_dict(),
            "data/cache/loss/epoch%03d-loss%.3f-val_loss%.3f.pth"
...
def get_map(min_overlap, score_threshold):
    ...
    with open("data/cache/map/results.txt", "w") as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            f1 = (
                ...
                * 2
                / np.where(
                    (np.array(precision) + np.array(rec)) == 0,
                    1,
            ...
            if len(precision) > 0:
                ...
                recall_text = class_name + "\nrecall={:.2f}%".format(
                    rec[score_threshold_idx] * 100
                )
                precision_text = class_name + "\nprecision={:.2f}%".format(
                    precision[score_threshold_idx] * 100
                )
            else:
                f1_text = class_name + "\nF1=0.00"
                recall_text = class_name + "\nrecall=0.00%"
            ...
            log_avg_miss_rate, mr, false_pos_per_image = log_average_miss_rate(
    ...
    with open("data/cache/map/results.txt", "a") as results_file:
        ...
        for class_name in sorted(gt_counter_per_class):
            results_file.write(
                f"{class_name}: {str(gt_counter_per_class[class_name])}\n"
            )
    ...
    plot_title += (
        str(len(ground_truth_files)) + " images; " + str(n_classes) + " classes"
    )
    ...
    plot_color = "royalblue"
    draw_plot(
        ap_dict, n_classes, window_title, plot_title, x_label, output_path, plot_color
    )
    return mAP
...
def get_txt(seed, train_val_percent, train_percent):
    ...
    for image_set in ["train", "val"]:
        image_ids = (
            open(
                f"data/VOCdevkit/VOC2007/ImageSets/Main/{image_set}.txt",
                encoding="utf-8",
            )
            .read()
            .strip()
        ...
        for image_id in image_ids:
            ...
            in_file = open(
                f"data/VOCdevkit/VOC2007/Annotations/{image_id}.xml", encoding="utf-8"
            )
...
def print_table(table_data, col_widths):
    ...
    for i in range(len(table_data[0])):
        print(
            f"| {table_data[0][i].rjust(col_widths[0])} | {table_data[1][i].rjust(col_widths[1])} |"
        )
    print("-" * (sum(col_widths) + 7))
...
def warm_cos_lr(
    lr, min_lr, total_iter, warmup_total_iter, warmup_lr_start, no_aug_iter, _iter
):
    if _iter <= warmup_total_iter:
        lr = (lr - warmup_lr_start) * pow(
            _iter / float(warmup_total_iter), 2
        ) + warmup_lr_start
        ...
        lr = min_lr + 0.5 * (lr - min_lr) * (
            ...
            + math.cos(
                math.pi
                * (_iter - warmup_total_iter)
                / (total_iter - warmup_total_iter - no_aug_iter)
            )
        )
    return lr
