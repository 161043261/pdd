...
async def image(file: UploadFile):
    ...
    if extend_name.lower() in ["jpeg", "jpg", "png"]:
        ...
        os.rename(
            f"data/cache/video/out/{_time_str}.avi",
            f"data/cache/video/out/{file_name}.avi",
        )
...
if __name__ == "__main__":
    mode = input(
        "Input mode(app, directory, fps, k-means, map, onnx, backbone, summary, camera): "
    )
    ...
    if mode == "app":
        ...
        for image_name in tqdm(image_names):
            ...
            os.makedirs("data/cache/image/out", exist_ok=True)
            image.save(
                f"data/cache/image/out/{image_name.rsplit('.', 1)[0]}.png",
                quality=95,
                subsampling=0,
            )
        ...
        get_txt(0, 0.9, 0.9)
        image_ids = (
            open("data/VOCdevkit/VOC2007/ImageSets/Main/test.txt")
            .read()
            .strip()
            .split()
        )
        ...
        for image_id in tqdm(image_ids):
            ...
            with open(f"data/cache/map/ground-truth/{image_id}.txt", "w") as new_f:
                root = ElementTree.parse(
                    f"data/VOCdevkit/VOC2007/Annotations/{image_id}.xml"
                ).getroot()
                for obj in root.findall("object"):
                    ...
                    if difficult_flag:
                        new_f.write(
                            f"{obj_name} {left} {top} {right} {bottom} difficult\n"
                        )
                    else:
