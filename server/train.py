...
if __name__ == "__main__":
    ...
    min_lr_fit = min(
        max(batch_size / nbs * init_lr * 0.01, lr_limit_min * 1e-2), lr_limit_max * 1e-2
    )
    pg0, pg1, pg2 = [], [], []
    for k, v in model.named_modules():
        ...
        if isinstance(v, nn.BatchNorm2d) or "bn" in k:
            ...
            pg1.append(v.weight)
    optimizer = {
        "adam": optim.Adam(pg0, init_lr_fit, betas=(0.937, 0.999)),
        "adam_w": optim.AdamW(pg0, init_lr_fit, betas=(0.937, 0.999)),
        "sgd": optim.SGD(pg0, init_lr_fit, momentum=0.937, nesterov=True),
    }[optimizer_type]
    ...
    val_dataset = YoloDataset(
        val_lines, num_classes, epoch, False, False, 0, 0, False, 0
    )
    gen = DataLoader(
        train_dataset,
        shuffle=True,
    ...
    for e in range(init_epoch, epoch):
        if e >= 50 and not unfreeze_flag and freeze_train:
            ...
            min_lr_fit = min(
                max(batch_size / nbs * init_lr * 0.01, lr_limit_min * 1e-2),
                lr_limit_max * 1e-2,
            )
